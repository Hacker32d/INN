...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white-slate backdrop-blur-md border-b border-slate-200/80 transition-all">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-20">
        
        {/* Logo con Isotipo */}
        <Link to="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-xl bg-slate-900 group-hover:bg-green-600 transition-colors flex items-center justify-center text-white shadow-sm">
            <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
              <path d="M12 3L2 12h3v8h6v-6h2v6h6v-8h3L12 3z" />
            </svg>
          </div>
          <span className="text-2xl font-black tracking-tight text-slate-900">
            LUX <span className="text-green-600">INMOBILIARIA</span>
          </span>
        </Link>
        ...
        <div className="hidden md:flex items-center space-x-9 text-sm font-semibold tracking-wide text-slate-600">
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className={`flex items-center gap-1.5 py-2 transition-colors focus:outline-none ${
                isProyectosActive ? 'text-green-600 font-bold' : 'hover:text-green-600'
              }`}
            >
              <span>Proyectos</span>
              <svg
                className={`w-4 h-4 transition-transform duration-200 ${
                  dropdownProyectos ? 'rotate-180 text-green-600' : ''
                }`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
          ...
          <Link
            to="/nosotros"
            className={`transition-colors py-2 relative ${
              isActive('/nosotros') ? 'text-green-600 font-bold' : 'hover:text-green-600'
            }`}
          >
            Nosotros
            {isActive('/nosotros') && (
          ...
          <Link
            to="/contacto"
            className={`transition-colors py-2 relative ${
              isActive('/contacto') ? 'text-green-600 font-bold' : 'hover:text-green-600'
            }`}
          >
            Contacto
            {isActive('/contacto') && (
      ...
      {menuAbierto && (
        <div className="md:hidden bg-gray-400/98 backdrop-blur-lg border-b border-slate-200 px-6 py-8 animate-in slide-in-from-top duration-300">
            ...
            <div className="flex flex-col items-center space-y-4 w-full">
              <Link
                ...
                className={`text-base font-bold transition-colors ${
                  isActive('/') ? 'text-green-600' : 'text-slate-800 hover:text-green-600'
                }`}
              >
                Inicio
              </Link>
              <Link
                to="/nosotros"
                className={`text-base font-bold transition-colors ${
                  isActive('/nosotros') ? 'text-green-600' : 'text-slate-800 hover:text-green-600'
                }`}
              >
                Nosotros
              </Link>
              <Link
                to="/contacto"
                className={`text-base font-bold transition-colors ${
                  isActive('/contacto') ? 'text-green-600' : 'text-slate-800 hover:text-green-600'
                }`}
              >
                Contacto
              </Link>
